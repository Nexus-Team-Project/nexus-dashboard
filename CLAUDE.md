...
**Note (2026-06-30) - the offer-level validity-type card is GONE; validity TYPE is chosen per inventory batch.** Supersedes the 2026-06-25, 2026-06-25c, and 2026-06-28 notes' references to `VoucherValidityTypeCard`/`defaultValidityType` in authoring. The validity TYPE (`limit` vs `from_until`) is now picked per BATCH in `VoucherInventoryModal`, so the offer-level default was removed from the Create/Edit forms: `VoucherValidityTypeCard.tsx` is deleted, the `defaultValidityType` form state is gone from `CreateOffer`/`EditOffer`, and it is no longer sent in the create/edit FormData (dropped from `createOfferFormData.ts` and `EditOffer` fd.append/load). `VariantsManager` no longer takes a `defaultValidityType` prop - it passes a fixed `defaultType="limit"` to `StagedInventoryModal` as the batch popup's initial toggle (the admin switches per batch). `CreateOffer.handleImport` no longer sets a validity type (imported units carry their own validity). The backend `defaultValidityType` field and the READ-ONLY validity-type display in `BenefitsPartnerships`/`OfferModal` (`validityTypeLabel`) are untouched - existing offers still show their stored type, new offers show a dash. Kept i18n: `co_validityTypeCardTitle` (now only the batch popup's group aria-label), `co_validityTypeLimit`, `co_validityTypeFromUntil`. Removed orphaned i18n: `co_validityTypeCardHint` + the `validityType` `FieldInfoKey` and its `fi_validityType_*` strings.
...
**Note (2026-06-30) - voucher variant pricing UI: one "Sale Price" column; "member price" is never shown.** The Benefits Partnerships variant table/card (`VariantDetailsTable` in `BenefitsPartnerships.tsx`) shows only **Sale Price + Value**. The label "member price"/"מחיר לחבר" appears NOWHERE (the `co_variantPriceLabel` key was deleted; `VariantList.tsx`'s create-form chip now uses `bp_variantSellingPrice`). The editable price column is titled **"Sale Price"** (`fi_nexusCost_label` was relabeled to "Sale Price"/"מחיר מכירה"; its tooltip now reads "the price this voucher is sold to customers for on the platform"). The separate Nexus-price column was removed; the slider popover (`VoucherPricePopover`) still edits the per-tenant `member_price` bounded `[nexus_cost, face_value]` (so the displayed value starts at the base nexus price and is adjustable up to face value), and that value is what shows as the voucher price across Benefits + Product Catalog. Popover copy was relabeled: `vp_title` "Set sale price", `vp_membersPay` "Customers pay" (HE "הלקוח משלם"), keeping the `vp_yourProfit` row; `vp_clickToEdit` "Click to set sale price". **Edit Offer modal:** for `tenant_only` offers the owning tenant's catalog managers may edit the variant's real **sale price (nexus_cost) + face value** - `EditOffer` computes `pricingLocked = !(isPlatformAdmin || (offer.visibility === 'tenant_only' && canManageSupply))` and threads it through `VariantsManager` -> `VariantBuilder` (disables both price inputs when locked). Ecosystem offers stay platform-admin-locked. The backend (`supply.service.ts` `updateOffer` / `tenant-pricing.service.ts`) is the real boundary. On a deal-price change it re-syncs per-tenant overrides by visibility: **tenant_only** SNAPS the displayed Sale Price to the new value (`resetTenantPricesForChangedVariants` drops the `variantPrices` override for each changed variant + clears the legacy offer-level `memberPrice` so it falls back to the reseeded base = the new sale price); **ecosystem** only clamps adopters' overrides into bounds (`clampTenantVariantPricesToBounds`). No frontend change is needed for the snap - the Benefits page re-fetches on navigation (see the root CLAUDE.md Voucher pricing rule).

**Note (2026-07-01) - the per-tenant price is now a stored markup PERCENTAGE (Phase 1).** `VoucherPricePopover` sets a per-tenant, per-variant markup **%** on the offer's base sale price, not an absolute price: `effective = min(base*(1+pct/100), face_value)` (agorot), % range `[0, maxPct]` with `maxPct = (face/base-1)*100`. It has a % slider + a type-in % box (synced), a persistent **Base sale price + Value** line, live **הלקוח משלם** (effective) + **הרווח שלך** (effective - nexus_cost) rows, and the shared `FieldTooltip` (`fieldKey` `voucherMarkupPct`, short text + "למד עוד" -> `FieldInfoModal`). Props are `baseSalePrice` + `currentMarkupPct` (was `currentMemberPrice`); on save it sends `updateTenantVoucherPrice(offerId, markupPct, variantId)` (api body `{ markupPct, variantId }`) and `onSaved(effective)`. The base + current % come from the catalog read's new `CatalogVariant.baseMemberPrice` + `tenantMarkupPct`. The Benefits table **main-row voucher price** now shows the per-variant **effective** range (lowest-highest) via `variantMemberPriceRange` for ANY variant count (single-variant shows its one effective price), with the optimistic `variantPriceOverrides` folded in (shared `applyVariantOverrides` helper). Backend: `TenantOfferConfig.variantMarkupPct`, route body `markupPct`, `supply-price.helper.ts` markup math. **Phase 2 (planned, not built):** separate adopted/non-adopted views (adopted first, non-adopted lazy, rate-limited), adoption = a frozen versioned snapshot linked to the global id (inventory stays global), global edits show an "updated" indicator + change list + a re-adopt (delete+recreate) button instead of auto-propagating.
...
- **User-facing messages use ONE component (sonner toast).** Every transient success / error / warning / info message goes through the app's single sonner toaster - `import { toast } from 'sonner'` then `toast.success` / `toast.error` / `toast.warning` / `toast.info`. The one `<Toaster richColors position="top-center" expand visibleToasts={5} />` is mounted in `App.tsx`; do not mount another. NEVER use an ad-hoc inline banner/alert box, `window.alert`, `window.confirm`, or a one-off notice for a transient message. (Persistent contextual notices that are part of a page's content - e.g. the Edit-offer denial-reason banner - are not transient messages and stay inline.) For a backend error, resolve the display string with `localizedApiError(err, language)` (`src/lib/apiError.ts`) so it shows the backend `errorHe` in Hebrew mode; every non-ok API response already throws an `ApiError` carrying both `message` (English) and `messageHe`. Migrate any surviving inline message banner to the toast when you touch it.
