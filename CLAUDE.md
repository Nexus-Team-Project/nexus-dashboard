...
**Note (2026-06-28) - voucher CSV bulk replaced by XLSX variant/inventory import; Sale Price may equal Value.** OpenSpec `voucher-xlsx-variant-import`. The voucher **CSV bulk upload was removed entirely** from `/supply/create` (deleted `VoucherCsvBulk`, `CreationModeTabs`, `lib/voucherCsv`, `bulkCreateVouchers` + its types, and the `vb_*` i18n; shared `lib/csvParser.ts` kept for member invite/contacts). It is replaced by an **Excel (XLSX) import where one row = one barcoded inventory unit** (not one offer). New entry: `VoucherImportControls` renders **Manual** + **Import** buttons (voucher-only) where the toggle was; Import opens an inline popover with **CSV (disabled/soon)** and **Excel (XLSX)**. XLSX opens `VoucherImportModal` (upload -> map -> match): real Excel is read client-side by `lib/xlsxReader.ts` (SheetJS `xlsx`, the ONLY real Excel reader in the app - `csvParser` is text-only); `VoucherColumnMapping` (a clone of `/users` `components/ColumnMapping.tsx`, leaving it untouched) maps columns to **Value (`face_value`), Sale price (`nexus_cost`), Barcode (required), Allow-stackable + Expiry (optional)**; when the stackable column is mapped, `StackableValueMatch` maps its 1-2 distinct free-text values to yes/no (**>2 distinct values is a blocking error**). `pages/voucherXlsxImport.ts` `rowsToDraftVariants` groups rows by **(Value, Sale price, stackable)** - the group key mirrors `draftSignature` so generated variants are never mutual duplicates - and turns each row into one `kind:'barcode'` staged unit; **Value/Sale-price coupling** (mapping one fills both); unmapped stackable -> variant `stackable` left **unset** (the existing mandatory gate forces a choice before publish, no default). Dates are normalized client-side by `lib/voucherImportDates.ts` (`normalizeExpiry` handles US `M/D/YYYY` + time, `D-MMM-YYYY`, `MM/DD/YYYY`, ISO, Excel serials -> `YYYY-MM-DD`; missing/unparseable -> **today + 5 years**, `validFrom` = import date, so every imported unit is `from_until`). On confirm, `CreateOffer.handleImport` appends the generated `DraftVariant[]` into the form (rendered by the existing `VariantsManager`), sets the offer `defaultValidityType = 'from_until'`, and toasts counts + any duplicate-barcode / skipped-row warnings; publish rides the **unchanged** per-variant `stagedUnitsToBatches` -> `addVariantInventory` path (NO backend/endpoint/schema change). **Price rule relaxed:** a variant's Sale price (`nexus_cost`) may now **equal** the Value (`face_value`), only blocked when it exceeds it - enforced in `voucherVariantDraft.ts` (`validateVariantDraft` + `nexusPriceError`) AND the backend `validateVoucherVariants` in `offers.routes.ts` (the one backend edit). New i18n keys are `vxi_*` (EN + HE). **Orphaned:** the backend CSV bulk path (`POST /api/v1/offers/bulk`, `voucher-bulk.service.ts`, `supply.ingest`) now has no caller - left in place, cleanup is a follow-up. **No test runner in the repo** - the import/date/grouping logic is pure and was verified by throwaway scripts; adding vitest is a pending decision.
