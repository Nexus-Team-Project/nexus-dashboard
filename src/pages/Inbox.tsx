...
const Inbox = () => {
  ...
  const [leftWidth, setLeftWidth] = useState(288); // w-72 = 288px
  ...
  // Resize drag handler for left & right panels
  useEffect(() => {
    if (!resizing) return;
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      rafId = requestAnimationFrame(() => {
        ...
        if (resizing === 'left') {
          // RTL: left panel is on the right side of the screen
          const newW = rect.right - e.clientX;
          if (newW >= 200 && newW <= 420) setLeftWidth(newW);
        } else {
          // RTL: right (info) panel is on the left side
          const newW = e.clientX - rect.left;
          if (newW >= 200 && newW <= 420) setRightWidth(newW);
        }
        rafId = null;
      });
    };
  ...
  // Make parent chain fill the viewport so Inbox can stretch to full height
  useEffect(() => {
    ...
    // Hide the footer so main gets the full height
  ...
  return (
    <div ref={containerRef} className="flex overflow-hidden h-full">
      {/* ── Left Sidebar: Conversation List ── */}
      <aside
        className={`flex flex-col border-e border-[#e3e8ee] dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden shrink-0 relative group/left ${resizing === 'left' ? '' : 'transition-[width] duration-200'}`}
        style={{ width: `${leftWidth}px` }}
      >
        {/* Resize Handle — inner edge (end side in RTL) */}
        <div
        ...
        >
          <div className={`absolute end-0 top-0 h-full transition-all duration-150 ease-out ${
            resizing === 'left'
              ? 'bg-[#635bff] w-0.5 shadow-lg shadow-primary/50'
              : 'bg-transparent w-0 group-hover/handle:bg-[#635bff] group-hover/handle:w-0.5 group-hover/handle:shadow-md group-hover/handle:shadow-primary/30'
          }`} />
        </div>
      ...
      <section className="flex flex-1 flex-col bg-slate-50 dark:bg-slate-900/40 overflow-hidden min-w-0">
        ...
        <header className="flex h-14 items-center justify-between border-b border-[#e3e8ee] dark:border-slate-700 bg-white dark:bg-slate-900 px-5 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`rounded-lg p-1 transition-colors ${showInfoPanel ? 'bg-[#635bff]/10 text-[#635bff]' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
            >
        ...
        {/* Message Input */}
        <div className="border-t border-[#e3e8ee] dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-2 shrink-0">
          <div className="flex flex-col rounded-lg border border-slate-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-[#635bff]/20 transition-all">
            {/* Textarea */}
            <textarea
              ref={textareaRef}
              value={messageText}
              onChange={(e) => setMessageText(e.target.value)}
              className="w-full border-none bg-transparent px-3 py-2 text-sm focus:ring-0 min-h-[36px] max-h-[60px] resize-none dark:text-white placeholder:text-slate-400 outline-none"
              placeholder={`כתוב הודעה ל${currentConv.name}...`}
              rows={1}
            />
            {/* Footer row: toolbar + actions */}
            <div className="flex items-center justify-between px-2 py-1.5">
              <div className="flex items-center gap-0.5">
                ...
                <div className="h-3.5 w-px bg-slate-200 dark:bg-slate-700 mx-0.5"></div>
                <button className="p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors rounded">
              ...
              <div className="flex gap-1.5">
                <button className="rounded-md bg-slate-100 dark:bg-slate-800 px-3 py-1 text-[11px] font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                  טיוטה
                </button>
                <button className="flex items-center gap-1.5 rounded-md bg-[#635bff] px-3.5 py-1 text-[11px] font-bold text-white hover:opacity-90 transition-opacity">
                  שלח
      ...
      <aside
        className={`flex flex-col border-s border-[#e3e8ee] dark:border-slate-700 bg-white dark:bg-slate-900 overflow-y-auto custom-scrollbar shrink-0 relative group/right ${
          resizing === 'right' ? '' : 'transition-all duration-300 ease-in-out'
        } ${
          showInfoPanel ? 'opacity-100' : 'w-0 opacity-0 overflow-hidden border-s-0'
        }`}
        style={showInfoPanel ? { width: `${rightWidth}px` } : undefined}
      >
        {/* Resize Handle — inner edge (start side in RTL) */}
        {showInfoPanel && (
          <div
          ...
          >
            <div className={`absolute start-0 top-0 h-full transition-all duration-150 ease-out ${
              resizing === 'right'
                ? 'bg-[#635bff] w-0.5 shadow-lg shadow-primary/50'
                : 'bg-transparent w-0 group-hover/handle:bg-[#635bff] group-hover/handle:w-0.5 group-hover/handle:shadow-md group-hover/handle:shadow-primary/30'
            }`} />
