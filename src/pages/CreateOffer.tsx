...
import VoucherImportControls from '../components/offer/VoucherImportControls';
import VoucherImportModal from '../components/offer/VoucherImportModal';
...
const CreateOffer = () => {
  ...
  /** Merges variants generated by the XLSX import into the form (group + inventory
   *  already built client-side). Imported units carry absolute from/until windows,
   *  so the offer's validity type becomes from_until. Warns on dup barcodes / skipped rows. */
  const handleImport = (outcome: VoucherImportOutcome) => {
    setShowImport(false);
    if (outcome.variants.length === 0) { toast.error(t('vxi_toastNoRows')); return; }
    setDefaultValidityType('from_until');
  ...
  return (
    <>
      ...
      {showImport && (
        <VoucherImportModal onClose={() => setShowImport(false)} onImport={handleImport} />
      )}
    </>
  );
};
